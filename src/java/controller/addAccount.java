...
public class addAccount extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        // Kiểm tra nếu tài khoản đã tồn tại chưa
        boolean isAccountCreated = acD.addAccount(a);
      
        // Xử lý sau khi đăng ký
        try ( PrintWriter out = response.getWriter()) {
            if (isAccountCreated) {
                out.println("<h2>Account created successfully!</h2>");
                response.sendRedirect("Login.jsp"); // Điều hướng người dùng đến trang login
            } else {
                out.println("<h2>Account creation failed. Try again!</h2>");
            }
        }
